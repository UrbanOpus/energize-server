...
var Account = new Schema({
    ...
    // User settings
    num_household_members: {type: Number},
    postal_code: {type: String},
    home_type: {type: String},
    home_size: {type: Number},

    // Energy account information
    gas_account_id: {type: String},
    gas_account_password: {type: String},
    electric_account_id: {type: String},
    electric_account_password: {type: String},
...
Account.statics.add_gas_account = function(token, accountid, password, cb) {
    var self = this;
    this.findOne({token: token}, function(err, usr) {
        if(err || !usr) {
            cb(err, null);
        } else {
            usr.gas_account_id = accountid;
            usr.gas_account_password = password;
...
Account.statics.add_electric_account = function(token, accountid, password, cb) {
    var self = this;
    this.findOne({token: token}, function(err, usr) {
        if(err || !usr) {
            cb(err, null);
        } else {
            usr.electric_account_id = accountid;
            usr.electric_account_password = password;
            usr.save(function(err, usr) {
                if (err) {
                    cb(err, null);
                } else {
